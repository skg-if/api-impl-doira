...
| `abstracts` | `descriptions[]` where `descriptionType == "Abstract"` | `abstract` (JATS-XML string) | ✅ [in](src/test/resources/datacite-esrf-dc-2493599001.json)·[out](src/test/resources/expected/datacite-product-esrf-dc-2493599001.json) | ✅\* [in](src/test/resources/crossref-journal-article-with-funder.json) (`CrossrefToSkgIfMapperTest.mapsAbstractStrippingJatsXmlTags`) | Crossref: XML tags stripped before use. The golden-tested Crossref fixture has no `abstract` at all - `abstracts` never appears in a Crossref golden output |
...
| `contributions[].by` (person) | `creators[]/contributors[].name/givenName/familyName/nameIdentifiers` (ORCID → full URL, else otf) | `author[]/editor[].given/family/orcid` (ORCID already a full URL; normalized to bare, then re-prefixed) | ✅ ORCID: [in](src/test/resources/datacite-esrf-dc-2493599001.json)·[out](src/test/resources/expected/datacite-product-esrf-dc-2493599001.json) | ❌ ORCID: not exercised for Product authors (all lack ORCID)<br>✅ otf fallback: [in](src/test/resources/crossref-journal-article.json)·[out](src/test/resources/expected/crossref-product-nature12373.json) (`CrossrefToSkgIfMapperTest.mapsAuthorsAsAuthorContributionsWithoutOrcidWhenAbsent`) | ORCID *is* demonstrated for Crossref in the Grant path (see Grant table) |
| `contributions[].declaredAffiliations` | `creators[]/contributors[].affiliation[]` (ROR if `affiliationIdentifierScheme == "ROR"`, else otf) | `author[]/editor[].affiliation[]` (name-only; always otf - Crossref carries no ROR here) | ❌ with ROR: no fixture has `affiliationIdentifierScheme`<br>✅ otf fallback: [in](src/test/resources/datacite-esrf-es-2210534378.json)·[out](src/test/resources/expected/datacite-product-esrf-es-2210534378.json) | ❌ both sub-cases: name-only affiliations exist in 2 fixtures but aren't asserted by any test; the golden-tested fixture's authors have empty `affiliation: []` | |
...
| `manifestations[].access_rights.status` | `rightsList[]` - `open` if any `rightsUri` contains `creativecommons.org` | `license[]` - `open` if any `url` contains `creativecommons.org` | ✅ [in](src/test/resources/datacite-esrf-dc-2493599001.json)·[out](src/test/resources/expected/datacite-product-esrf-dc-2493599001.json) | ✅\* [in](src/test/resources/crossref-journal-article-with-funder.json) (`CrossrefToSkgIfMapperTest.mapsAccessRightsAsOpenFromCreativeCommonsLicence`) | The golden-tested Crossref fixture's licence isn't CC, so its golden output shows `access_rights: {}`, never `open` |
...
| `manifestations[].biblio.issue/volume/pages` | *(not modeled)* | `issue`, `volume`, `page` (split on `-` into `first`/`last`) | – not modeled | ✅ [in](src/test/resources/crossref-journal-article.json)·[out](src/test/resources/expected/crossref-product-nature12373.json) | Crossref-only |
...
|---|---|---|---|---|---|
| `local_identifier` | `doi` (full URL) | `doi` (full URL) | ✅ [in](src/test/resources/datacite-award-ardc-83eg-9981.json)·[out](src/test/resources/expected/datacite-grant-award-ardc-83eg-9981.json) | ✅ [in](src/test/resources/crossref-grant.json)·[out](src/test/resources/expected/crossref-grant-218300.json) | |
