...
| `manifestations[].biblio.in` (venue) | *(not modeled)* | `container-title[0]` (otf id) + `ISSN[]` (`{scheme: "issn"}`) | – not modeled | ✅ [in](src/test/resources/crossref-journal-article.json)·[out](src/test/resources/expected/crossref-journal-article-out.json) | Crossref-only - DataCite has no equivalent venue field |
